package execution.datasources.hbase.types

import org.apache.spark.sql.Row
import execution.datasources.hbase._
...
/**
 * Currently, SHC supports three data types which can be used as serdes: PrimitiveType.
...
object SHCDataTypeFactory {
  ...
  def create(f: Field): SHCDataType = {
    ...
    if (f.fCoder == SparkHBaseConf.PrimitiveType) {
  ...
  def create(coder: String): SHCDataType = {
    ...
    if (coder == SparkHBaseConf.PrimitiveType) {
      new PrimitiveType()
    } else {
      // Data type implemented by user
      Class.forName(coder)
